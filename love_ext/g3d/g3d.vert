...
uniform mat4 projectionMatrix; // handled by the camera (camera "lens")
uniform mat4 viewMatrix;       // handled by the camera (camera position)
uniform mat4 modelMatrix;      // models send their own model matrices when drawn (model position)
...
vec4 position(mat4 transformProjection, vec4 vertexPosition) {
    ...
    screenPosition.y *= float(isCanvasEnabled)*-2.0+1.0; // convert to an equation to avoid branches
